...
def load_spatial_data():
    """Load site-level spatial data for mapping."""
    try:
        # Load the actual parquet file with spatial information
        df = pd.read_parquet("/outputs/phenology-intelligence-v1/parquet/phenology_data.parquet")
        
        # Aggregate to site level with required columns
        if "site_id" in df.columns and "latitude" in df.columns and "longitude" in df.columns:
            # Group by site level data and compute the required aggregations
            spatial_df = df.groupby(["site_id", "latitude", "longitude", "state"]).agg({
            ...
            spatial_df = spatial_df.rename(columns={
                ...
                "species_id": "distinct_species"
            })
            
            # Ensure numeric values
            spatial_df["observation_count"] = pd.to_numeric(spatial_df["observation_count"], errors="coerce").fillna(0)
            spatial_df["distinct_species"] = pd.to_numeric(spatial_df["distinct_species"], errors="coerce").fillna(0)
            
            # Filter out invalid coordinates
            spatial_df = spatial_df[(spatial_df["latitude"].notna()) & 
                       (spatial_df["longitude"].notna()) &
                       (spatial_df["latitude"] != 0) & 
                       (spatial_df["longitude"] != 0)]
            return spatial_df
        else:
            st.warning("Spatial data not found in parquet file")
            return pd.DataFrame()
...
st.write("Spatial map section loaded")

if not spatial_df.empty:
    try:
        # Use scatter_mapbox for interactive map - simpler approach
        fig_spatial = px.scatter_mapbox(
            ...
            color='state',
            hover_name='state',
            hover_data=['site_id', 'observation_count', 'distinct_species'],
            title='Spatial Distribution of Phenology Observations',
            zoom=3,
            height=600
        )
        ...
        st.subheader("Spatial Data Preview")
        st.dataframe(spatial_df.head(20))
    except Exception as e:
        st.warning(f"Error creating spatial map: {e}")
        st.write("Showing data preview instead:")
        st.dataframe(spatial_df.head(20))
else:
    st.write("No spatial data available for visualization.")
